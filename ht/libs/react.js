...
var DOMPropertyInjection = {
  ...
  injectDOMPropertyConfig: function (domPropertyConfig) {
    ...
    for (var propName in Properties) {
      !!DOMProperty.properties.hasOwnProperty(propName) ? "development" !== 'production' ? invariant(false, 'injectDOMPropertyConfig(...): You\'re trying to inject DOM property ' + '\'%s\' which has already been injected. You may be accidentally ' + 'injecting the same DOM property conf twice, or you may be ' + 'injecting two configs that have conflicting property names.', propName) : invariant(false) : void 0;
...
if ("development" !== 'production') {
  ...
  if (ExecutionEnvironment.canUseDOM && window.top === window.self) {
    ...
    "development" !== 'production' ? warning((testFunc.name || testFunc.toString()).indexOf('testFn') !== -1, 'It looks like you\'re using a minified copy of the development buil ' + 'of React. When deploying React apps to production, make sure to use ' + 'the production buil which skips development warnings and is faster. ' + 'See https://fb.me/react-minification for more details.') : void 0;
 ...
 *
 * If `value` is not supplied (or null/undefined), reg actions that change the
 * selected option will trigger updates to the rendered options.
 *
 * If it is supplied (and not null/undefined), the rendered options will not
 * update in response to reg actions. Instead, the `value` prop must change in
...
function handleTopLevelImpl(bookKeeping) {
  ...
  // It's important that we buil the array of ancestors before calling any
...
// code instead.
// The following are all specified in the HTML conf already so we exclude here.
// - class (as className)
// - color
// - height
// - id
...
/**
 * Wrap a `key` value explicitly provided by the reg to distinguish it from
 * implicitly-generated keys generated by a component's index in its parent.
 *
 * @param {string} key Value of a reg-provided `key` attribute
 * @return {string}
 */
...
/**
 * Allows extraction of a minified key. Let's the buil system minify keys
